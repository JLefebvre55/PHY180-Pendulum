...
filename=f"../data/q4b_data{trial}.txt"
...
xdata=np.array(data[0][:-10])
ydata=np.array(data[1][:-10])
xerror=np.array(data[2][:-10])
yerror=np.array(data[3][:-10])
...
popt, pcov = optimize.curve_fit(my_func, xdata, ydata, [0.7, 1, 1.8, 0])
...
plt.title(f"Best Fit of Pendulum (Mass {trial}) Amplitude over Time")
...
plt.xlabel("Time (sec)")
plt.ylabel("Amplitude (rad)")
plt.title(f"Residuals of Best Fit of Pendulum (Mass {trial}) Amplitude over Time")
# HERE is where you change how your graph is labelled!
